...
本文主要介绍了基本的CUDA程序框架及代码解析，包括如何申请设备指针内存、将数据拷贝到设备上、调用核函数进行计算、线程同步以及将结果拷贝回主机内等。同时也介绍了一些常见问题及解决方案，比如CUDA程序黑屏之后恢复的问题。接 {% post_link ai/cuda/记被CUDA折腾死去活来的那十天 %} 。阅读本文前需要有一部分C语言基础。
