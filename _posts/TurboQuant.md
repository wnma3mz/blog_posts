...
假设经过统计，得出最优的两个代表值是 `{-0.6, 0.6}`。这就是**码本（Codebook）**。

### 2. Quant（量化/编码）

接下来，把原向量 `[0.8, -0.5, 0.2, -0.9]` 中的每个数，**就近映射**到码本里，并记录它的**索引**：
...
- **IP-RMSE / IP-Bias（内积误差）**：在 LLM 的注意力机制和向量检索中，真正频繁做的是**点积计算**。
...
- **无损（Lossless）**：针对索引串 `1010`，如果能用类似 ZIP 压缩（如 Huffman 编码或 ANS 等信息论方法）把它压得更小，解压后依然是完美还原的 `1010` 索引串，这就叫无损压缩。
...
如果内积估计有系统偏差，在向量检索的 Top-K 排序或 Attention 打分中就会出现**持续的定向偏移**。
...
$$x \xrightarrow{R} y \xrightarrow{\text{量化}} \hat{y} \xrightarrow{R^\top} \hat{x}$$

1. **生成随机正交矩阵** $R \in \mathbb{R}^{d \times d}$（对高斯矩阵做 QR 分解）。
2. **旋转**：$y = Rx$。
3. **查表量化**：对每维做标量量化 $y_i \to c_{k^*}$，其中 $k^* = \arg\min_k |y_i - c_k|$。

   这里的码本 $C$ 就是针对 Beta 分布**预先离线计算好**的最优码本。
...
- 让逐维独立量化更合理：论文推导表明，随机旋转后各维度不仅分布稳定，而且近似**相互独立**。
...
假设用刚才讲的 TurboQuant_mse 算法，把 $x$ 量化成了 $\hat{x}_{\text{mse}} = [0.7, 0.6]$。
...
- 残差长度 $\gamma$：
  ...
  $$\gamma = \lVert r \rVert_2 = \sqrt{0.1^2 + (-0.1)^2} \approx 0.1414$$
...
1. 随机旋转：把残差 $r$ 乘以矩阵 $S$。
   ...
   $$S = \begin{bmatrix} 0.707 & -0.707 \\ 0.707 & 0.707 \end{bmatrix}$$
...
2. 再乘上缩放系数：
   ...
   $$\gamma \cdot \frac{\sqrt{\pi/2}}{d} = 0.1414 \cdot \frac{1.2533}{2} \approx 0.0886$$

   所以补丁向量约为

   $$\text{corr} \approx 0.0886 \cdot [1.414, 0] \approx [0.125, 0]$$

**把补丁加回去**

最终重建向量变成：

$$\hat{x}_{\text{prod}} = [0.7, 0.6] + [0.125, 0] = [0.825, 0.6]$$

如果查询向量仍取 $y = [1.0, 0.0]$，那么新的内积就是：

$$\langle y, \hat{x}_{\text{prod}} \rangle = [1.0, 0.0] \cdot [0.825, 0.6] = 0.825$$
