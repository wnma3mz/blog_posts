...
这两个放在一起的原因是，在IOS13之前，IPad其实差不多可以等同于大尺寸IPone。所以这里就是一个安卓手机（未ROOT）与IPone的介绍，这里的玩法也算是比较多的，下面一一介绍。关于APP的安装包在此就不提供了，请读者自行搜索。当然还有ADB这种玩法，也不做介绍，可参考 {% post_link engineering/iot/用ADB调试安卓手机 %}

#### Termux与JuiceSSH

放在搜索引擎一搜，其实这两个APP就有很多相关介绍了。但这里我想介绍一些不一样的玩法。关于 {% post_link engineering/iot/安卓手机利用Termux安装Linux 'Termux' %} 的基础配置，可以先看另一篇；这里只补充一些更偏玩法的内容。至于操作不方便这一点，可以使用JuiceSSH连接 localhost 来处理，具体操作不介绍。
...
**远程手机**：如何在手机不连接WIFI，不连接数据线的前提下，用其他设备访问手机（手机与其他设备的实际距离也可以很远），前提手机能上网。这一方法个人暂没有特别用途，但`Just for fun`。首先，需要一个公网IP服务器；接下来需要会玩ssh，`Termux`与服务器配置好ssh，`Termux`能够ssh到服务器上即可。最后一步，参考 {% post_link engineering/linux/ssh 'ssh笔记（端口转发）' %}。
